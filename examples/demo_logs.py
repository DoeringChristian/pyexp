...
def main():
    ...
    for i in range(100):
        ...
        loss = 1.0 * math.exp(-i / 30) + 0.1 * math.sin(i / 5) * math.exp(-i / 50)
        logger.add_scalar("loss", loss)

        # Simulated accuracy (increasing)
        acc = 1.0 - 0.9 * math.exp(-i / 25)
        ...
        if i % 25 == 0:
            try:
                ...
                center_x = 1.5 * math.exp(-i / 40)
                center_y = 1.5 * math.exp(-i / 40)
                Z = (
                    (X - center_x) ** 2
                    + (Y - center_y) ** 2
    ...
    logger.flush()
    print("Demo logs written to ./demo_logs")
    print("Run viewer with: uv run python -m pyexp.viewer demo_logs")
